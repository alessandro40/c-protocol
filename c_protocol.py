"""Create C handling functions for a network protocol"""
...
def field_read(variable, offset, size, indent, f_out):
    ...
    print('{}*{} = *((uint8_t *) pkt + {});'.format(indent, variable, offset),
          file=f_out)
    if size > 1:
        for i in range(1, size):
            print('{}*{} += (*((uint8_t *) pkt + {}) << {});'.format(
                indent, variable, offset + i, 8*i), file=f_out)

def field_write(variable, offset, size, indent, f_out):
    """"Write the code to write a field of the packet"""
    if size is 1:
        print('{}*((uint8_t *) (pkt + {})) = {};'.format(indent, offset,
                                                         variable), file=f_out)
    else:
        print('{}*((uint8_t *) (pkt + {})) = {} & 0xFF;'.format(
            indent, offset, variable), file=f_out)
        for i in range(1, size):
            print('{}*((uint8_t *) (pkt + {})) = ({} >> {}) & 0xFF;'.format(
...
def create_prototypes(prot, f_out):
...
def main_fn():
    ...
    parser = argparse.ArgumentParser(
        description='create C handling functions for a network protocol.')
    group = parser.add_mutually_exclusive_group()
    group.add_argument("-p", "--prototype",
                       help="create only the prototype of the functions",
                       action="store_true")
    group.add_argument("-i", "--implementation",
                       help="create only the implementation of the functions",
                       action="store_true")
    ...
    try:
        ...
        print('Decode of JSON failed', file=sys.stderr)
        return

    indent = 4*' '

    if args.prototype:
        create_prototypes(prot, f_out)
